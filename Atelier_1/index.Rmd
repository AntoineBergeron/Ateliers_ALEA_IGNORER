...
```
WHILE NbOrange_1 > 0 <--------------- Tant que le nombre d'orange de la caisse 1 est plus grand que 0
  NbOrange_1 <- NbOrange_1 - 1 <----- On enlève une orange de la caisse 1
  NbOrange_2 <- NbOrange_2 + 1 <----- On ajoute une orange dans la caisse 2
END WHILE  <------------------------- Se termine seulement quand le nombre d'orange de la caisse 1 sera plus petit ou égal à 0 (soit 0)
```
---
...
```
FOR numero_orange_1 IN 1:48 <--------------- Pour chacune des oranges (dénotées par un numéro de 1 à 48)
  NbOrange_1 <- NbOrange_1 - 1 <------------ On enlève une orange de la caisse 1
  NbOrange_2 <- NbOrange_2 + 1 <------------ On ajoute une orange dans la caisse 2
END FOR  
```
...
```
FOR nombre_interaction IN 1:(NbOrange_1/2) <---- Pour un nombre d'interaction de 1 à 24
  NbOrange_1 <- NbOrange_1 - 2 <----- On enlève deux orange de la caisse 1
  NbOrange_2 <- NbOrange_2 + 2 <----- On ajoute deux orange dans la caisse 2
END FOR  
```
...
- Ouvrir le fichier [ButsResult.csv](./donnees/ButsResult.csv) au moyen de Excel ou d'un éditeur de texte
...
- Calculer le nombre moyen de buts marqué par le Canadiens de Montréal pour chacun des résultats d'une partie (victoire, Nulle, Défaite). La fonction 'mean' et l'inscrire dans chaque position de `ButsMoyen`
...
```{r}
NHL <- read.table(file="donnees/Canadiens.csv", header=TRUE, stringsAsFactors=FALSE, sep = ";")
